 ...
 * @author Esraa
 */
public class AddCartMobile extends HttpServlet {
   ...
   public String state="the supplier company will deliver it";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            result = stmt.executeQuery("select id_mobile from cart where email_user='"+email_user+"'");
            Boolean flag = false;
            while(result.next()){
                isemptyflag = true;
                if(result.getInt(1)== parseInt(id_product)){
                //product exists then update
                        stmt2.executeUpdate("update cart set quantity=quantity+1 where id_mobile="+id_product+" AND email_user='"+email_user+"'");
                        flag = false;
                        break;
                    }
                flag = true;
            ...
            if(flag){
                stmt2.executeUpdate("insert into cart(id_mobile,email_user,quantity,order_status) values("+id_product+",'"+email_user+"',1,'"+state+"')");
            }
            if(!isemptyflag){
                stmt2.executeUpdate("insert into cart(id_mobile,email_user,quantity,order_status)values("+id_product+",'"+email_user+"',1,'"+state+"')");
            }
