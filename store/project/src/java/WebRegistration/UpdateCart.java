    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            stmt2 = c.createStatement();
                ResultSet result = stmt.executeQuery("select quantity_product from mobile_product where id_product="+p_id);
            while(result.next()){
                Integer prod_quantity = result.getInt(1);
                if(parseInt(new_quantity)<prod_quantity){
                stmt2.executeUpdate("update cart set quantity="+parseInt(new_quantity)+" where email_user='"+email_user+"' AND id_mobile="+p_id);
                }
